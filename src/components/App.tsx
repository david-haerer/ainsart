...
class DayBadge extends TimeBadge {
  ...
  get labelBottom() { return this.zdt.toLocaleString('de-DE', { day: 'numeric', month: "numeric" }) }
...
class DaytimeBadge extends TimeBadge {
  ...
  get labelTop() { return `${this.zdt.toLocaleString('de-DE', { day: 'numeric', month: "long" })}, ${this.start.hour}:00 - ${this.end.hour}:00 Uhr` }
  get labelBottom() { return `${this.start.hour} - ${this.end.hour} Uhr` }
}
...
export default function App() {
  ...
  //   const scaleDiff = scale / initialScaleRef.current
  //   const newZoom = scaleDiff > 1.2 ? Math.min(ZOOM_LEVELS.length - 1, timeline.current.zoom + 1) : (scaleDiff < 0.8) ? Math.max(0, timeline.current.zoom - 1) : timeline.current.zoom
  ...
  return (
    <main className="h-[calc(100dvh-40px-4px-22px-4px-22px-4px)]">
      ...
      <div
        ref={containerRef}
        className="w-full h-[4px+22px+4px+22px+4px] overflow-hidden select-none cursor-grab border-solid border-gray-300 dark:border-gray-700"
        style={{
          touchAction: 'none',
      ...
      >
        <svg width={getVisibleWidth()} height={p + h + p + h + p} className="block" style={{ backgroundColor: 'transparent' }}>
          {layout.bottom.map((badge) => (
            <foreignObject key={badge.id} x={badge.x(layout.startMilliseconds, layout.ppd)} y={p} width={badge.width(layout.ppd)} height={h}>
          ...
          {layout.top.map((badge) => (
            <foreignObject key={badge.id} x={badge.x(layout.startMilliseconds, layout.ppd)} y={p + h + p} width={badge.width(layout.ppd)} height={h}>
              <div className="w-full h-full flex items-center justify-center">
                <Badge variant={badge.isPast(layout.nowMilliseconds) ? 'past' : badge.isFuture(layout.nowMilliseconds) ? 'future' : 'present'} className="flex w-full">
                  {badge.labelTop}
                </Badge>
